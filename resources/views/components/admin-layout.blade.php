@php
    $user = Auth::user();
    $roleLabel = $user?->hasRole('staff') ? 'Staff Console' : 'Admin Console';
    ...
    $userInitials = $nameParts->count() >= 2
        ...
        : mb_strtoupper(mb_substr($nameParts->first() ?? 'A', 0, 1));
    $sidebarCollapsed = request()->cookie('admin.sidebar.collapsed') === 'true';
...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="bg-slate-100 font-sans antialiased text-slate-900 overflow-hidden">
        <div data-admin-shell data-admin-shell-key="admin" data-admin-sidebar-collapsed="{{ $sidebarCollapsed ? 'true' : 'false' }}" class="admin-shell">
            <div
                data-admin-overlay
            ...
            >
                <div class="flex items-center justify-between border-b border-white/10 px-6 py-5">
                    <a href="{{ route('admin.dashboard') }}" class="admin-sidebar-brand flex min-w-0 items-center gap-3">
                        ...
                        <div class="admin-sidebar-brand-copy min-w-0" data-admin-sidebar-text>
                            <p class="truncate text-sm font-semibold tracking-wide text-white">VerifyHomes Admin</p>
                ...
                <div class="admin-sidebar-scroll flex-1 overflow-y-auto px-4 py-6">
                    <div class="admin-sidebar-summary rounded-2xl border border-white/10 bg-white/5 p-4" data-admin-sidebar-summary>
                        <p class="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Operations Menu</p>
                        <p class="mt-2 text-sm text-slate-300">Review trusted accounts, listings, and inspection activity from one workspace.</p>
                    </div>
                                    ...
                                    @switch($link['icon'])
                                        ...
                                        @case('landlords')
                                            <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true">
                                                ...
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M17.25 16.5l1.5 1.5 3-3" />
                                            </svg>
                                            @break
                                        @case('tenants')
                                            <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true">
                                        ...
                                        @case('audit')
                                            <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true">
                                                ...
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M12 3l7.5 3v5.25c0 4.733-3.188 9.005-7.5 10.5-4.312-1.495-7.5-5.767-7.5-10.5V6L12 3z" />
                                            </svg>
                                            @break
                                        @case('payments')
                                            <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true">
                                        ...
                                        @case('purchases')
                                            <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M3.75 7.5h16.5l-1.5 9.75H5.25L3.75 7.5z" />
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M8.25 20.25a.75.75 0 110-1.5.75.75 0 010 1.5z" />
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M16.5 20.25a.75.75 0 110-1.5.75.75 0 010 1.5z" />
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M6.75 7.5l-.75-3h-2.5" />
                                            </svg>
                                            @break
                                        @case('occupancy')
                                            <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true">
                    ...
                    <div class="border-t border-white/10 px-4 py-5">
                        <div class="admin-sidebar-account rounded-2xl bg-white/5 p-4">
                            ...
                            <div class="admin-sidebar-account-actions mt-4 flex gap-2">
                                <a href="{{ route('profile.edit') }}" class="inline-flex flex-1 items-center justify-center rounded-xl border border-white/10 px-3 py-2 text-sm font-medium text-slate-200 hover:bg-white/5">
            ...
            <div class="admin-main">
                <header class="admin-topbar">
                    <div class="admin-topbar-inner">
                        <div class="flex min-w-0 items-center gap-3">
                            ...
                            <div class="min-w-0">
                                ...
                                <div class="flex min-w-0 items-center gap-3">
                                    ...
                                    @else
                                        <h1 class="admin-topbar-title truncate">VerifyHomes Admin</h1>
                                    @endauth
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-3">
                            @if ($quickSearchRoute)
                                <form method="GET" action="{{ $quickSearchRoute }}" class="hidden md:flex">
                                    <label class="admin-topbar-search">
                                        <span class="sr-only">Search admin workspace</span>
                                        <svg class="admin-topbar-search-icon h-4 w-4" viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M14.5 14.5L18 18" />
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M9 15.5A6.5 6.5 0 109 2.5a6.5 6.5 0 000 13z" />
                                        </svg>
                                        <input type="search" name="q" class="admin-topbar-search-input" placeholder="Search properties, landlords, payments" />
                                    </label>
                                </form>
                            @endif

                            @if (! empty($pageActionLabel) && ! empty($pageActionHref))
                                <a href="{{ $pageActionHref }}" class="admin-topbar-action hidden xl:inline-flex">
                                    {{ $pageActionLabel }}
                                </a>
                            @endif
                            ...
                            <button type="button" class="admin-topbar-icon-button hidden sm:inline-flex" aria-label="Theme preference">
                                <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M21 12.79A9 9 0 1111.21 3c-.14.45-.21.93-.21 1.43A7.57 7.57 0 0018.57 12c.5 0 .98-.07 1.43-.21z" />
                                </svg>
                            </button>
                            ...
                            @auth
                                <div class="admin-topbar-profile" data-admin-profile>
                                    ...
                                    <div class="admin-topbar-dropdown hidden" data-admin-profile-menu>
                                        ...
                                        <div class="admin-topbar-dropdown-links">
                                            <a href="{{ route('profile.edit') }}" class="admin-topbar-dropdown-link">Profile</a>
                                            <a href="{{ route('profile.edit') }}#settings" class="admin-topbar-dropdown-link">Settings</a>
                                        </div>
